...
function AddCompanyPages() {
  ...
  const holidayOption = useMemo(() => {
    ...
    const handleHolidayListSubmit = (e) => {
      ...
      if (companyPageData.company_name !== '' && companyPageData.company_pagename && companyPageData.company_pagetype !== '' && companyPageData.company_pagestatus !== '' && addHolidays.holidaylist_title !== '' && addHolidays.holiday_date.length !== 0) {
       toast.promise(
            axios.post('/api/addcompanypageholidays', { 'companypagedata': companyPageData, 'pagedetails': {...addHolidays,department:addHolidays.department.map(dep=>dep.value)} }),
            {
              pending: {
                render() {
                  return ('Adding CompanyPage Details')
    ...
    return (
      <>
        <Paper elevation={5} sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: { xs: '60ch', md: '52ch', lg: '52ch' } }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', p: 1, width: '100%' }}>
            <Stack component={'form'} onSubmit={handleHolidayListSubmit} spacing={2} sx={{ mb: 2, borderBottom: '1px solid black', width: '100%' }} p={1} >
  ...
  }, [addHolidays, holidayData, tempHolidayData, companyPageData,inputValue])
