...
function ViewCompanyPages() {
				...
				return (
					<>
						...
						<Paper elevation={5} sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', width: { xs: '35ch', md: '50ch', lg: '50ch' }, height: { xs: '55ch', sm: '55ch', md: '55ch' } }}>
							...
							<Typography variant='p' component={'h5'} m={1} >Title: {viewCompanyPageData.companyPageData[0].holidaylist_title}</Typography>
							<Typography variant='p' component={'h5'}  >Departments: <span style={{ color: 'gray' }}> {viewCompanyPageData.companyPageData[0].department}</span></Typography>
	...
	const holidayOption = useMemo(() => {
		...
		return (
			<>
				<Paper elevation={1} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: { sm: '50ch', md: '50ch', lg: '60ch' }, minHeight: { md: '45ch' } }}>

					<Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', width: { sm: '45ch', lg: '55ch' }, minHeight: { md: '45ch' }, p: 1 }}>
						<Stack spacing={{ xs: 2, sm: 2, md: 2, lg: 2 }} sx={{ mb: 2, borderBottom: '1px solid black', width: '100%' }} p={1} >
							<FormControl fullWidth variant="outlined">
								...
								<Autocomplete
									...
									value={addHolidays.department}
									isOptionEqualToValue={(option, value) => option.value === value.value}
									onChange={(_, newValue) => {
										//console.log(newValue);
										setAddHolidays({ ...addHolidays, department: newValue })
									}}
									...
									style={{ width: 'auto' }}
									renderInput={(params) => (
										<TextField required={addHolidays.department.length === 0}  {...params} size='small' label="Department" />
									)}
								/>
							</FormControl>
	...
	}, [addHolidays, tempHolidayData, holidayData, inputValue])
			...
			axios.post('/api/getcompanypagedata', row)
				.then(res => {
					...
					setAddHolidays({
						...
						department: options.filter(dep => res.data[0].department.split(',').includes(dep.value)),
	...
	const companyPageEditView = useMemo(() => {
		...
		const handleEditForm = (e) => {
			...
			let method;
			if (compDetails.company_pagetype === 'Holidays' && addHolidays.holidaylist_title !== '' && addHolidays.department.length!==0 && addHolidays.holiday_date.length !== 0) {
				method = axios.put('/api/updatecompanypageholidays', { compDetails, editHolidays: {...addHolidays,department:addHolidays.department.map(dep=>dep.value)} })
				//console.log(addHolidays, compDetails.company_pagetype === 'Holidays' && addHolidays.holidaylist_title !== '' && addHolidays.department.length!==0 && addHolidays.holiday_date.length !== 0)
			}
				...
				>
					...
					<DialogContent dividers={true}>
						...
						<Container sx={{ display: 'flex', flexDirection: { xs: 'column', lg: 'row' } }} >
							...
							<Paper elevation={1} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: { sm: '40ch', md: '40ch', lg: '40ch' }, minHeight: { sm: '30ch', md: '45ch' } }}>

								<Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', width: { sm: '35ch', md: '35ch' }, minHeight: { sm: '25ch', md: '45ch' }, p: 1 }}>
